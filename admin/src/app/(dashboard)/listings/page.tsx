/**
 * Listing Approvals Page
 * 
 * Review and approve/reject user-submitted listing suggestions.
 * Will be fully implemented in Task 2.12.
 */
export default function ListingsPage() {
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-[var(--text-primary)]">
          Listing Approvals
        </h1>
        <p className="text-sm text-[var(--text-muted)] mt-1">
          Review and approve suggested listings
        </p>
      </div>

      <div className="bg-white dark:bg-[var(--surface)] rounded-2xl p-8 border border-[var(--border-default)] text-center">
        <div className="text-4xl mb-4">📍</div>
        <h2 className="text-lg font-semibold text-[var(--text-primary)]">
          Coming in Task 2.12
        </h2>
        <p className="text-sm text-[var(--text-muted)] mt-2 max-w-md mx-auto">
          This page will show pending listing suggestions with preview details 
          and approve/reject actions.
        </p>
      </div>
    </div>
  );
}
